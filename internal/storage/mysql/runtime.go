...
const runtimeConfigColumns = `qps, burst, window_seconds, ip_qps, ip_burst, global_qps, global_burst,
	...
	COALESCE(ip_blocklist,''), COALESCE(ip_whitelist,''), record_args, updated_at`
...
func (s *Store) GetRuntimeConfig(ctx context.Context) (*model.RuntimeConfig, bool, error) {
	...
	err := s.db.QueryRowContext(ctx,
		...
		&blocklistJSON, &whitelistJSON, &recordArgs, &cfg.UpdatedAt)
	...
	cfg.Observability = &model.RuntimeObservability{RecordArgs: recordArgs != 0}
	return &cfg, true, nil
}
	...
	_, err = s.db.ExecContext(ctx,
		`INSERT INTO runtime_config
		   (id, qps, burst, window_seconds, ip_qps, ip_burst, global_qps, global_burst,
		    auto_ban_enabled, auto_ban_window_seconds, auto_ban_max_violations, auto_ban_ban_seconds,
		    ip_blocklist, ip_whitelist, record_args, updated_at)
		 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
		 ON DUPLICATE KEY UPDATE
		   ...
		   ip_blocklist=VALUES(ip_blocklist), ip_whitelist=VALUES(ip_whitelist),
		   record_args=VALUES(record_args), updated_at=VALUES(updated_at)`,
		...
		nullIfEmpty(blocklistJSON), nullIfEmpty(whitelistJSON), recordArgs, fmtTimeUTC(nowOr(cfg.UpdatedAt).Time))
	if err != nil {
